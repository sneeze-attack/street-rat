...
export default class RollResults extends Phaser.GameObjects.Group {
  ...
  constructor(scene, statOrSkill, playerNumber, reason) {
    ...
    const resultsShadeBox = scene.add.rectangle(0, 0, config.scale.width, config.scale.height, 0x000000).setOrigin(0, 0).setDepth(0).setAlpha(0.5);
    ...
    const rollTitle = scene.add.text(((config.scale.width * phx) / 128), ((config.scale.height * 17) / 128), `Roll Against ${statOrSkill}`).setColor('#0033FF').setFontFamily('"DejaVu Sans Mono"').setFontSize(32).setDepth(0);
    const rollReason = scene.add.text(((config.scale.width * 54.5) / 128), ((config.scale.height * 26.5) / 128), `(${reason})`).setColor('#A9A9A9').setFontFamily('"DejaVu Sans Mono"').setFontSize(24).setDepth(0);

    // add dice images
    const defaultOne = 'dice_one';

    const diceOne = scene.add.sprite(((config.scale.width * 50) / 128), ((config.scale.height * 40) / 128), defaultOne).setOrigin(0, 0);
    const diceTwo = scene.add.sprite(((config.scale.width * 60) / 128), ((config.scale.height * 40) / 128), defaultOne).setOrigin(0, 0);
    const diceThree = scene.add.sprite(((config.scale.width * 70) / 128), ((config.scale.height * 40) / 128), defaultOne).setOrigin(0, 0);
    ...
    const scoreText = scene.add.text(((config.scale.width * 40) / 128), ((config.scale.height * 57) / 128), 'You').setColor('#33FF00').setFontFamily('"DejaVu Sans Mono"').setFontSize(40).setDepth(0);
    const scoreNumber = scene.add.text(((config.scale.width * 38) / 128), ((config.scale.height * 70) / 128), scoreOffsetText).setColor('#33FF00').setFontFamily('"DejaVu Sans Mono"').setFontSize(80).setDepth(0);

    const vsText = scene.add.text(((config.scale.width * 58.5) / 128), ((config.scale.height * 63) / 128), 'vs').setColor('#FFFFFF').setFontFamily('"DejaVu Sans Mono"').setFontSize(80).setDepth(0);

    const rollText = scene.add.text(((config.scale.width * 80) / 128), ((config.scale.height * 57) / 128), 'Total').setColor('#A00000').setFontFamily('"DejaVu Sans Mono"').setFontSize(40).setDepth(0);
    const rollTotalText = scene.add.text(((config.scale.width * 80) / 128), ((config.scale.height * 70) / 128), '0').setColor('#A00000').setFontFamily('"DejaVu Sans Mono"').setFontSize(80).setDepth(0);
    ...
    const successOrFailure = 'Success';
    const successOrFailureText = scene.add.text(((config.scale.width * 52) / 128), ((config.scale.height * 93.5) / 128), successOrFailure).setColor('#A9A9A9').setFontFamily('"DejaVu Sans Mono"').setFontSize(56).setDepth(0);

    const firstLine = new Phaser.Geom.Line(((config.scale.width * 30) / 128), ((config.scale.height * 34) / 128), ((config.scale.width * 98) / 128), ((config.scale.height * 34) / 128));
    ...
    const masterBox = scene.add.rectangle(0, 0, config.scale.width, config.scale.height, 0x000000).setOrigin(0, 0).setDepth(0).setAlpha(0.01).setInteractive();
  ...
  rollTheDice() {
    ...
    const resultThree = (Math.floor((Math.random() * 6) + 1));
