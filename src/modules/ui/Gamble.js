...
export default class Gamble extends Phaser.GameObjects.Group {
  ...
  constructor(scene) {
    ...
    const backButton = scene.add.text(((config.scale.width * 24) / 128), ((config.scale.height * 101) / 128), 'Back').setColor('#FFFFFF').setInteractive().setFontSize(32);
    ...
    const rouletteText = scene.add.text(((config.scale.width * 21) / 128), ((config.scale.height * 22) / 128), 'Roulette').setColor('#FFFFFF').setInteractive().setFontSize(32);
